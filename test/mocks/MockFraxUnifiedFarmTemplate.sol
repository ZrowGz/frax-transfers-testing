...
import "@frax_testing/gauges/Math/Math.sol";
import "@frax_testing/gauges/Curve/IveFXS.sol";
import "@frax_testing/gauges/Curve/IFraxGaugeController.sol";
import "@frax_testing/gauges/Curve/IFraxGaugeFXSRewardsDistributor.sol";
import "@frax_testing/gauges/Uniswap/TransferHelper.sol";
// import "frax_testing/gauges/ERC20/IERC20.sol";
import "@openzeppelin-contracts/contracts/token/ERC20/utils/SafeERC20.sol";
import "@frax_testing/gauges/Utils/ReentrancyGuard.sol";
import "@frax_testing/gauges/Utils/Owned.sol";

import "./MockGaugeController.sol";
import "./MockRewardsDistributor.sol";

// Extra rewards
// import "frax_testing/gauges/Misc_AMOs/convex/IConvexBaseRewardPool.sol";

contract FraxUnifiedFarmTemplate is ReentrancyGuard, Owned {
    /* ========== STATE VARIABLES ========== */
    ...
    function getRewardTokenAt(uint256 i) external view returns (address) {
        return rewardTokens[i];
    }

    // Last time the reward was applicable
    function lastTimeRewardApplicable() internal view returns (uint256) {
        uint256 val = Math.min(block.timestamp, periodFinish);
        return val;
    }
    ...
    {
        // revert("Need cCCW logic"); /// Overridden in specific version - reduce "unreachable code" warnings.
    }
    ...
    function fraxPerLPToken() public virtual returns (uint256) {
    ...
    function veFXSMultiplier(address account) public view returns (uint256 vefxs_multiplier) {
        ...
        /// @dev Hardcoded in the minted balance veFXS due to it calling our nonexistent contract on mainet
        vefxs_bal_to_use = (the_proxy == address(0)) ? veFXS.balanceOf(account) : 1001000000000000000000000; //veFXS.balanceOf(the_proxy);
        ...
        uint256 mult_optn_1 = ((vefxs_bal_to_use * vefxs_max_multiplier * vefxs_boost_scale_factor) /
            1006000000000000000000000) *
            /// @dev modified this to have the amount of assets minted in the test represented, otherwise resulted in 0
            MULTIPLIER_PRECISION; //(veFXS.totalSupply() * MULTIPLIER_PRECISION);
    ...
    function _getRewardExtraLogic(address rewardee, address destination_address) internal virtual {
        // rewardee;
        // destination_address;
        // revert("Need gREL logic"); /// overridden in specific version - silenced to reduce "unreachable code" warnings
    }
    ...
    function retroCatchUp() internal {
        ...
        for (uint256 i = 0; i < rewardTokens.length; i++) {
            uint256 amountToPay = (rewardRates(i) * rewardsDuration * (num_periods_elapsed + 1));
            uint256 amountOnHand = IERC20(rewardTokens[i]).balanceOf(address(this));
            require(
                amountToPay <= amountOnHand,
